...
$descriptions = array(
                      ...
                      "wt" => "含子函数<br>执行时间<br>(微秒)",
                      "IWall%" => "IWall%",
                      "excl_wt" => "本函数<br>执行时间<br>(微秒)",
                      "EWall%" => "EWall%",

                      "ut" => "Incl. User<br>(微秒)",
                      "IUser%" => "IUser%",
                      "excl_ut" => "Excl. User<br>(微秒)",
                      "EUser%" => "EUser%",
                      ...
                      "cpu" => "Incl. CPU<br>(微秒)",
                      "ICpu%" => "ICpu%",
                      "excl_cpu" => "Excl. CPU<br>(微秒)",
                      "ECpu%" => "ECPU%",

                      "mu" => "含子函数<br>使用内存<br>(bytes)",
                      "IMUse%" => "IMemUse%",
                      "excl_mu" => "本函数<br>使用内存<br>(bytes)",
                      "EMUse%" => "EMemUse%",

                      "pmu" => "函子函数<br>内存峰值<br>(bytes)",
                      "IPMUse%" => "IPeakMemUse%",
                      "excl_pmu" => "本函数<br>内存峰值<br>(bytes)",
                      "EPMUse%" => "EPeakMemUse%",
...
$diff_descriptions = array(
                      ...
                      "wt" => "Incl. Wall<br>Diff<br>(微秒)",
                      "IWall%" => "IWall<br> Diff%",
                      "excl_wt" => "Excl. Wall<br>Diff<br>(微秒)",
                      "EWall%" => "EWall<br>Diff%",

                      "ut" => "Incl. User Diff<br>(微秒)",
                      "IUser%" => "IUser<br>Diff%",
                      "excl_ut" => "Excl. User<br>Diff<br>(微秒)",
                      "EUser%" => "EUser<br>Diff%",

                      "cpu" => "Incl. CPU Diff<br>(微秒)",
                      "ICpu%" => "ICpu<br>Diff%",
                      "excl_cpu" => "Excl. CPU<br>Diff<br>(微秒)",
                      "ECpu%" => "ECpu<br>Diff%",

                      "st" => "Incl. Sys Diff<br>(微秒)",
                      "ISys%" => "ISys<br>Diff%",
                      "excl_st" => "Excl. Sys Diff<br>(微秒)",
                      "ESys%" => "ESys<br>Diff%",
...
function profiler_report ($url_params,
  ...
  if ($diff_mode) {
    ...
    $run1_link = xhprof_render_link('查看运行 #' . $run1,
                           "$base_path/?" .
                           http_build_query(xhprof_array_set($base_url_params,
                                                      'run',
                                                      $run1)));
    $run2_txt = sprintf("<b>运行 #%s:</b> %s",
                        $run2, $run2_desc);

    $run2_link = xhprof_render_link('查看运行 #' . $run2,
                                    "$base_path/?" .
                        http_build_query(xhprof_array_set($base_url_params,
                                                          'run',
                                                          $run2)));
  ...
  $links = array();
  $links [] =  xhprof_render_link("查看顶级${diff_text}报告",
  ...
  echo
    ...
    '  </dd>' .
    '  <dt>提示：</dt>' .
    '  <dd>点击函数名深入往下跟踪。<a href="/index.php?datei='.$_GET['datei'].'">【返回列表页】</a></dd>' .
    '</dl>' .
...
function print_flat_data($url_params, $title, $flat_data, $sort, $run1, $run2, $limit) {
  ...
  if (!$limit) {              // no limit
    ...
    $display_link = xhprof_render_link(" [ <b class=bubble>显示所有 </b>]",
                                       "$base_path/?" .
                                       http_build_query(xhprof_array_set($url_params,
                                                                         'all', 1)));
  }
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2) {
  ...
  if ($diff_mode) {
    ...
    print("<h3><center>整体的Diff摘要</center></h3>");
    print('<table border=1 cellpadding=2 cellspacing=1 width="30%" '
          .'rules=rows bordercolor="#bdc7d8" align=center>' . "\n");
    print('<tr bgcolor="#bdc7d8" align=right>');
    print("<th></th>");
    print("<th $vwbar>" . xhprof_render_link("运行 #$run1", $href1) . "</th>");
    print("<th $vwbar>" . xhprof_render_link("运行 #$run2", $href2) . "</th>");
    ...
    $callgraph_report_title = '[使用调用图的Diff查看回归模型/改进]';
    ...
    echo "<tr>";
    echo "<th style='text-align:right'>总体概述</th>";
    echo "<th></th>";
    echo "</tr>";

    foreach ($metrics as $metric) {
      echo "<tr>";
      echo "<td style='text-align:right; font-weight:bold'>总 "
    ...
    if ($display_calls) {
      echo "<tr>";
      echo "<td style='text-align:right; font-weight:bold'>函数调用数：</td>";
    ...
    $callgraph_report_title = '[查看全部调用图]';
  }
    /*
  print("<center><br><h3>" .
  ...
  if ($diff_mode) {
    if ($all) {
      ...
      $title = "Top 100 <i style='color:red'>回归</i>/"
               . "<i style='color:green'>改进</i>: "
               . "Sorted by $desc Diff";
    }
  } else {
    if ($all) {
      ...
      $title = "显示前 $limit 个函数: 按照 $desc 排序";
    }
  }
...
function symbol_report($url_params,
  ...
  if ($diff_mode) {
    ...
    print("<th $vwbar><a href=" . $href1 . ">运行 #$run1</a></th>");
    print("<th $vwbar><a href=" . $href2 . ">运行 #$run2</a></th>");
    print("<th $vwbar>Diff</th>");
    print("<th $vwbar>Diff%</th>");
    print('</tr>');
    print('<tr>');
...
function displayXHProfReport($xhprof_runs_impl, $url_params, $source,
                             $run, $wts, $symbol, $sort, $run1, $run2,$datei) {
  ...
  if ($run) {                              // specific run to display?
    ...
    echo "No XHProf runs specified in the URL.";
      for ($i = 0; $i < 7; $i++) {
          $datei = date('Ymd', strtotime('-' . $i . ' day'));
          echo '【<a href="/index.php?datei=' . $datei . '">' . $datei . '</a>】';
      }
